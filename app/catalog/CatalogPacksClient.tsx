...
export default function CatalogPacksClient() {
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10">
      ...
      {/* Filters (internal navigation OK) */}
      <div className="mt-8 flex flex-wrap gap-2">
        ...
        <Link
          href="/catalog/packs?genre=rnb"
          className="rounded-full border bg-white px-4 py-2 text-sm font-semibold"
        >
          R&amp;B
        </Link>
        <Link
          href="/catalog/packs?genre=soul"
          className="rounded-full border bg-white px-4 py-2 text-sm font-semibold"
        >
          Soul
        </Link>
      ...
      <div id="packs" className="mt-10 space-y-16">
        {showJazz && (
          <PackGrid label="Jazz Packs" packs={jazzPacks} refPrefix="jazz" />
        )}
        {showRnb && (
          <PackGrid label="R&B Packs" packs={rnbPacks} refPrefix="rnb" />
        )}
      ...
      {/* One global bundle CTA at bottom */}
    ...
    <section>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {packs.map((pack) => {
          ...
          // Optional tracking param (safe)
          const checkoutUrl = withQuery(PACK_BUNDLE_STRIPE_LINK, {
            client_reference_id: `catalog_packs_${refPrefix}_${pack.slug}`,
          });
              ...
              <div className="mt-4 grid gap-3">
                ...
                {samples.length < 2 && (
                  <div className="text-xs opacity-60">
                    Add 2 tracks to this pack in <code>app/catalog/packs/packs.ts</code>{" "}
                    so it always shows two previews.
                  </div>
                )}
              </div>
